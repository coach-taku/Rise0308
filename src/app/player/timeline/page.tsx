...
import { Profile, DailyRecordWithProfile, Comment } from '@/types/database'
import { getGroupMembers, getAllDailyRecords, addComment, getProfiles } from '@/lib/data'
...
export default function TimelinePage() {
  ...
  const [records, setRecords] = useState<DailyRecordWithProfile[]>([])
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const [allRecords, profiles] = await Promise.all([
          getAllDailyRecords(startDate),
          getProfiles(),
        ])

        // Filter records for group members (or all if coach)
        let filteredRecords = allRecords
        if (userData.role === 'player' && userData.group_id) {
          const groupMembers = await getGroupMembers(userData.group_id)
          const memberIds = new Set(groupMembers.map(m => m.id))
          filteredRecords = allRecords.filter(r => memberIds.has(r.user_id))
        }
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
                          isOwnRecord ? 'bg-brand-main text-brand-dark' : 'bg-gray-200 text-gray-600'
                        }`}>
                        ...
                        {comments.length > 0 && (
                          <div className="space-y-2 mb-3">
                            {comments.map(comment => {
                              const commenterName = comment.profiles?.name || getProfileName(comment.user_id)
                              const isCoachComment = getProfileRole(comment.user_id) === 'coach'
                              return (
                                <div key={comment.id} className={`px-3 py-2 rounded-lg text-xs ${
                                  isCoachComment ? 'bg-blue-50 border border-blue-100' : 'bg-gray-50'
                                }`}>
                                  <div className="flex items-center gap-1 mb-1">
                                    <span className="font-semibold text-gray-700">{commenterName}</span>
                                    {isCoachComment && (
                                      <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full">コーチ</span>
                                    )}
                                  </div>
