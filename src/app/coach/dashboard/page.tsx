...
import { Profile, DailyRecordWithProfile, Tournament } from '@/types/database'
import { getProfiles, getAllDailyRecords, getActiveTournament, addComment } from '@/lib/data'
...
export default function CoachDashboard() {
  ...
  const [todayRecords, setTodayRecords] = useState<DailyRecordWithProfile[]>([])
  const [recentRecords, setRecentRecords] = useState<DailyRecordWithProfile[]>([])
  ...
  useEffect(() => {
    ...
    if (userData.role !== 'coach') { router.push('/player/dashboard'); return }
    setUser(userData)
  ...
  const loadData = async () => {
    try {
      ...
      const [profiles, records, t] = await Promise.all([
        getProfiles(),
      ...
      setTodayRecords(records.filter(r => r.target_date === today))
    } catch (e) {
      console.error(e)
    } finally {
      setLoading(false)
  ...
  const handleDateChange = (date: string) => {
    ...
    const filtered = recentRecords.filter(r => r.target_date === date)
  ...
  return (
    <div className="min-h-screen bg-brand-bg pb-20 md:pb-8">
      <Header userName={user.name} role="coach" />
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className={`font-bold ${
                            displayRecord.fatigue_level >= 8 ? 'text-red-500' : displayRecord.fatigue_level >= 6 ? 'text-orange-500' : 'text-green-500'
                          }`}>
                        ...
                        {displayRecord.has_pain && (
                          <div className="bg-red-50 px-2 py-1.5 rounded-lg mt-1">
                            <p className="text-xs text-red-600 font-medium">⚠️ 痛みあり: {displayRecord.pain_details}</p>
                          </div>
                        )}
                      </div>
                    ) : (
          ...
          <div className="space-y-3">
            {recentRecords.length === 0 ? (
              ...
              recentRecords.slice(0, 20).map(record => {
                const profileName = record.profiles?.name || getProfileName(record.user_id)
                ...
                return (
                  <div key={record.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <div className="p-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">
                              {format(parseISO(record.target_date), 'M月d日(E)', { locale: ja })}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                      ...
                      {record.selected_goals.length > 0 && (
                        <div className="flex flex-wrap gap-1 mb-2">
                          {record.selected_goals.map((goal, idx) => (
                      ...
                      <div className="flex items-center gap-2 mt-3 text-xs text-gray-500">
                        ...
                        {record.has_pain && <span className="text-red-500">⚠️ {record.pain_details}</span>}
                      </div>
                    </div>
                              ...
                              {comments.map(comment => (
                                <div key={comment.id} className="bg-blue-50 border border-blue-100 px-3 py-2 rounded-lg text-xs">
                                  <span className="font-semibold text-gray-700">
                                    {comment.profiles?.name || getProfileName(comment.user_id)}
                                  </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {players.map(player => {
                ...
                const avgEval = playerRecords.length > 0
                  ...
                  : '-'
                const totalPoints = playerRecords.reduce((sum, r) => sum + r.earned_points, 0)
                ...
                return (
                  <div key={player.id} className="bg-white rounded-xl p-4 shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-semibold text-gray-800">{player.name}</h4>
                      <span className="text-xs bg-brand-main text-brand-dark px-2 py-1 rounded-full font-medium">
