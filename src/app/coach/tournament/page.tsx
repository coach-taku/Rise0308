...
export default function TournamentPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        setTournaments(prev => prev.map(t => t.id === editingId ? result : (formActive ? { ...t, is_active: false } : t)))
      } else {
        if (formActive) {
          setTournaments(prev => [...prev.map(t => ({ ...t, is_active: false })), result])
        } else {
  ...
  const handleEdit = (tournament: Tournament) => {
    ...
    setFormDate(tournament.target_date)
    setFormActive(tournament.is_active)
    setShowForm(true)
  }
        ...
        <div className="space-y-3">
          {tournaments.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-sm">まだ大会が登録されていません</p>
            </div>
          ) : (
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-2xl p-4 shadow-sm ${
                    tournament.is_active ? 'border-2 border-brand-main' : ''
                  }`}
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {tournament.is_active && (
                          <span className="text-xs bg-brand-main text-brand-dark px-2 py-0.5 rounded-full font-medium">
                            現在の目標
                          </span>
                        )}
                      </div>
                    ...
                    <div className="text-right">
                      {!isPast ? (
                        <>
                          <p className="text-2xl font-bold text-brand-main">{daysRemaining}</p>
                          <p className="text-xs text-gray-500">日後</p>
                        </>
                      ) : (
                        <p className="text-sm text-gray-400">終了</p>
                      )}
                    </div>
                  </div>
                  <button
                    onClick={() => handleEdit(tournament)}
                    className="mt-3 text-xs text-gray-500 hover:text-brand-main transition-colors"
                  >
                    編集する →
                  </button>
                </div>
